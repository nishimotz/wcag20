  	...
  	<xsl:result-document method="xml" href="{$output.dir}/{$filename}" encoding="UTF-8" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" indent="no">
  		<html>
  			...
  			<head>
  				...
  				<title>
  					<xsl:value-of select="head"/> | WCAG 2.0解説書
  				</title>
  		...
  		<html>
  			...
  			<head>
  				...
  				<title>
  					...
  					<xsl:value-of select="head"/> を理解する | WCAG 2.0解説書
  				</title>
  				<xsl:call-template name="css"/>
  				<link rel="stylesheet" type="text/css" href="slicenav.css"/>
...
<!--Generate the appendices -->   
  ...
  <xsl:template name="navigation.top">
    ...
    <ul id="navigation">
      ...
      <li>
        <strong><a href="intro.html" title="Introduction to Understanding WCAG 2.0">イントロダクション</a></strong>
      </li>
      <xsl:choose>
        <xsl:when test="$prev">
          <li>
      ...
      <xsl:when test="$target/@id">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$target/@id='conformance-referencing'"> 他の文書からの WCAG 2.0 の参照方法</xsl:when>
           <xsl:when test="$target/@id='accessibility-support-documenting'"> ウェブ技術の使用法のアクセシビリティ・サポートを文書化する</xsl:when>
    ...
    <h3 id="{ancestor::div2/@id}-{$anchorinfo}-head" class="div3head" >
      ...
      <xsl:choose>
        ...
        <xsl:when test="../@role='failures'">
				 達成基準 <xsl:call-template name="sc-number"><xsl:with-param name="id" select="../../../@id"/></xsl:call-template> のよくある不適合事例
			</xsl:when>
        <xsl:when test="../@role='tech-optional'">
				 達成基準 <xsl:call-template name="sc-number"><xsl:with-param name="id" select="../../../@id"/></xsl:call-template> でさらに対応が望まれる実装方法（参考）
        </xsl:when>
        <xsl:when test="../@role='sufficient'">
				</xsl:when>
...
<strong>使用法：</strong> そのコンテンツに合致する状況を以下から選択すること。それぞれの状況には、<acronym title="Web Content Accessibility Guidelines">WCAG</acronym> ワーキンググループがその状況において十分であると判断する、番号付の実装方法（又は、実装方法の組合せ）がある。
</p>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
