...
<xsl:apply-templates></xsl:apply-templates>
    ...
    <xsl:when test="$slices='1'"><h3 class="small-head" id="{../../@id}-ex{$exnumber}">事例<xsl:value-of select="$exnumber"></xsl:value-of><xsl:if test="head[@role='fail']"><xsl:text> (failure)</xsl:text></xsl:if><xsl:if test="head"><xsl:text>: </xsl:text></xsl:if> <xsl:value-of select="head"></xsl:value-of></h3></xsl:when>
			  <xsl:when test="$bytech='1'"><h4 class="small-head" id="{../../@id}-ex{$exnumber}">事例<xsl:value-of select="$exnumber"></xsl:value-of><xsl:if test="head[@role='fail']"><xsl:text> (failure)</xsl:text></xsl:if><xsl:if test="head"><xsl:text>: </xsl:text></xsl:if> <xsl:value-of select="head"></xsl:value-of></h4></xsl:when>
    <xsl:otherwise><h5 class="small-head" id="{../../@id}-ex{$exnumber}">事例<xsl:value-of select="$exnumber"></xsl:value-of><xsl:if test="head[@role='fail']"><xsl:text> (failure)</xsl:text></xsl:if><xsl:if test="head"><xsl:text>: </xsl:text></xsl:if> <xsl:value-of select="head"></xsl:value-of></h5></xsl:otherwise>
  </xsl:choose>
...
<xsl:apply-templates></xsl:apply-templates>
	...
	<xsl:template match="resources">
		<!--<div class="resources">
          ...
          <xsl:otherwise>4</xsl:otherwise>
			...
			<xsl:otherwise><p>この実装方法に関するリソースはない。</p></xsl:otherwise>
			</xsl:choose>
			</div>
		<!--</div>-->
	</xsl:template>
	...
	<xsl:when test="$bytech='1'">3</xsl:when>
		...
		<div class="textbody">
			...
			<xsl:otherwise><p>(現時点では無し)</p></xsl:otherwise>
			</xsl:choose>
			</div>
		<!--</div>-->
	</xsl:template>
			    ...
			    <xsl:when test="child::node()">
			      <xsl:apply-templates/>
			    	...
			    	<xsl:if test="not(ancestor::div1[@id = 'failures'])">
			    		<p>注意： この実装方法が「達成基準を満たすことのできる実装方法」の一つである場合、このチェックポイントや判定基準を満たしていなければ、それはこの実装方法が正しく用いられておらず、この実装方法によって達成基準を満たしていないことを意味するが、必ずしも達成基準を満たしていないことにはならない。場合によっては、別の実装方法によってその達成基準が満たされていることもありうる。</p>
			    	</xsl:if>
			    </xsl:when>
...
<xsl:apply-templates select="issue"/>			
	...
	<xsl:template match="applies-to">
			...
			<xsl:element name="p">
				...
				<xsl:attribute name="class">referenced</xsl:attribute>この<xsl:choose>
      <xsl:when test="ancestor::div1[@role='failures']">不適合事例</xsl:when>
      <xsl:otherwise>実装方法</xsl:otherwise>
    </xsl:choose>は、以下に関連するものである:</xsl:element>
			<ul>
			...
			<ul>
				<li>
				    <a href="{$quickref-src//publoc/loc[@href]}#{$id}">
						How to Meet <xsl:call-template name="sc-number"><xsl:with-param name="id" select="$id"/></xsl:call-template> (<xsl:call-template name="sc-handle"><xsl:with-param name="handleid" select="$id"/></xsl:call-template>): 英語
					</a>
				</li>
				<li>
				    <a href="{$guide-src//publoc/loc[@href]}{$filename}{fragment}">
