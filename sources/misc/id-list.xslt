...
$itemnumber_array[0] = "(none selected)";
				<xsl:for-each select="$techniques.doc//spec/body/div1">
							...
							<optgroup label="{$head}">
								 <xsl:for-each select="technique">
										$itemnumber_array[<xsl:value-of select="(count(preceding::technique)+1)"/>] = "<xsl:value-of select="@id"></xsl:value-of>: <xsl:value-of select="substring(short-name, 1, 85)"/>...";</xsl:for-each>
										</optgroup>
								</xsl:for-each>
								</select>
				</p>
